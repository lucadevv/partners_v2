...
/// Bottom sheet de subcategorías: GET /options/subcategories/{category_id}, infinite scroll y búsqueda.
class SubcategoryBottomSheetContent extends StatefulWidget {
  final String categoryId;
  final CreateBranchFormNotifier formNotifier;
  final VoidCallback onSelect;

  const SubcategoryBottomSheetContent({
    super.key,
  ...
  @override
  State<SubcategoryBottomSheetContent> createState() =>
      _SubcategoryBottomSheetContentState();
}

class _SubcategoryBottomSheetContentState
    extends State<SubcategoryBottomSheetContent> {
  ...
  List<SubcategoryEntity> get _displayList {
    if (_keyword.isEmpty) return _subcategories;
    final lower = _keyword.toLowerCase();
    final local = _subcategories
        .where((s) => s.name.toLowerCase().contains(lower))
        .toList();
    return local.isNotEmpty ? local : _subcategories;
  }
  ...
  void _onSearchChanged() {
    ...
    _searchDebounce = Timer(_searchDebounceDuration, () {
      ...
      final localMatch =
          _subcategories.any((s) => s.name.toLowerCase().contains(lower));
  ...
  Future<void> _loadPage(int page, {String? keyword}) async {
    ...
    final result = await getIt<GetSubcategoriesUsecase>().call(
      widget.categoryId,
      page,
      keyword: keyword,
    );
    ...
    result.fold((_) => setState(() => _isLoading = false), (paginated) {
      setState(() {
        if (page == 1) _subcategories.clear();
        _subcategories.addAll(paginated.data);
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: CreateBranchScreenStrings.subcategorySearchHint,
                    hintStyle: TextStyle(
                ...
                child: _displayList.isEmpty && !_isLoading
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final sub = _displayList[index];
                          return Material(
                            color: Colors.transparent,
                            child: InkWell(
                              onTap: () {
                                widget.formNotifier.setSubCategory(sub);
                                widget.onSelect();
                              },
                              child: Padding(
